namespace jojCore {
    export class AiMedium implements IAi {
        ...
        addBestMoviment(bestMoviments: IMove[][][], positionsAHead: number,
            ...
            nivel: number, isBlack: boolean): IMove[][][] {
            ...
            let positionsAHeadOnMoviment =
                BoardHelper.getY0Start7End(nextPosition.y, isBlack)
            ...
            let bestMovimentIndex
                = bestMoviments[nivel][indexPositionsAHead].findIndex(
                    bestMoviment =>
                        bestMoviment.startPosition.x === startPosition.x
        ...
        getBestNivel(bestMoviments: IMove[][][]): number {
            let bestNivel = 0;
        ...
        fillPiecesWhereCanTheyGo(game: IGame, pieces: IGamePiece[],
            isBlack: boolean): IGamePiece[] {
        ...
        //Need teste
        getWinPosition(winPositions, x: number, isBlack: boolean)
            : IMove {
        ...
        getDistance(piece: IGamePiece, isBlack: boolean)
            : number[] {
        ...
        getNMovimentsToWin(pieces: IGamePiece[], isBlack: boolean)
            : IGamePiece[][][] {

            pieces.forEach(piece =>
                piece.movimentsToWin = this.getDistance(piece, isBlack));

            let winPositions: IGamePiece[][][] = [];
            for (let x = 0; x < 8; x++) {
                if (!winPositions[x])
                    winPositions[x] = [];
        ...
        isGameEnding(allPieces: IGamePiece[], isBlack: boolean)
            : boolean {
        ...
        getLastMovimentToWin(nMovimentsToWin: IGamePiece[][]): number {
        ...
        getPiecesFurtherAway(pieces: IGamePiece[], isBlack: boolean)
            : number {
        ...
        getBestMoviment(game: IGame, moves: IMove[], isBlack: boolean)
            : IMove {
        ...
        getMoveNearWinPosition(allPieces: IGamePiece[],
            moviments: IMove[], orderedPieces: IGamePiece[][],
            winPositions, isBlack: boolean,
            game: IGame): IMove {

            let winMove: IMove = undefined;
        ...
        addBestMovimentsAfterMove(piece: IGamePiece,
            startPositionY: number, startPositionBeforeMove: IGamePosition,
            nextPositionBeforeMove, game: IGame, isBlack: boolean,
            bestMoviments, winPositions) {
        ...
        getBestMove(game: IGame): IMove {
            ...
            let move = this.getMoveNearWinPosition(allPieces, higherBestMoviments,
